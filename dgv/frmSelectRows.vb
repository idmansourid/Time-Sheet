...
Public Class frmSelectRows
    ...
    Public Event _List_SelectedIndexChanged(sender As frmSelectRows, OnDgv As uDgv)
    Public Event _List_DoubleClickIfNotError(sender As frmSelectRows, OnDgv As uDgv)
...
#End Region
    ...
    Private Sub frmValidOfColumn_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        chkLstFiltered.Dock = DockStyle.Fill
        lstFiltered.Show()
        trvFiltered.Hide()
        chkLstFiltered.Hide()
    End Sub
    Public Enum ShowMode
        ...
        CheckBoxList = 2
    End Enum
    Public Sub Show_TrvLstChk(inMode As ShowMode, Optional InData As String() = Nothing)
        Dim Obj() As Control = {lstFiltered, trvFiltered, chkLstFiltered}
        Obj.ToList.ForEach(Sub(d) d.Visible = False)
        Obj(inMode).Visible = True
        Select Case inMode
            Case ShowMode.List
                TryCast(Obj(inMode), ListBox).Items.Clear()
                TryCast(Obj(inMode), ListBox).Items.AddRange(InData)
            Case ShowMode.CheckBoxList
                TryCast(Obj(inMode), CheckedListBox).Items.Clear()
                TryCast(Obj(inMode), CheckedListBox).Items.AddRange(InData)
        End Select
    End Sub
    ...
    Private Sub tsButOk_Click(sender As Object, e As EventArgs) Handles tsButOk.Click
        Me.Hide()
    End Sub

    Private Sub tsButCancel_Click(sender As Object, e As EventArgs) Handles tsButCancel.Click
        Me.Hide()
    End Sub
